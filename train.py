...
from utils import ChestXrayDataset,ImgNor_6
...
group.add_argument('--train-file', default='train.txt', type=str,
                   help='Training data txt file.')
group.add_argument('--val-file', default='val.txt', type=str,
                   help='Validation data txt file.')
...
def main():
    ...
    dataset_train = ChestXrayDataset(os.path.join(args.data_dir, args.train_file), phase='train', normalization=ImgNor_6, transform=train_transform, target_transform=None)
    dataset_eval = ChestXrayDataset(os.path.join(args.data_dir, args.val_file), phase='val', normalization=ImgNor_6, transform=eval_transform, target_transform=None)
    ...
    loader_eval = DataLoader(dataset_eval, batch_size=args.validation_batch_size, shuffle=False, num_workers=args.workers)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        input, target = input.float().cuda(), target.cuda()
